...
def main():
    ...
    for sector in stock.domestic_stock:
        stock_type = "domestic"
...
with open('domestic_stock_description.pkl', 'wb') as f:
    pickle.dump(result, f)

# pkl 파일을 읽어 Python 코드로 저장
with open('domestic_stock_description.pkl', 'rb') as f:
    loaded_data = pickle.load(f)

# UTF-8 인코딩으로 .py 파일로 저장
with open('domestic_stock_description.py', 'w', encoding='utf-8') as f:
    f.write("# -*- coding: utf-8 -*-\n")  # UTF-8 선언
    f.write(f"data = {repr(loaded_data)}\n")  # 데이터 저장

print("Data has been saved to domestic_stock_description.py")